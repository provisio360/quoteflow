...
The competitive spread reported for a [[Benchmark Item]]: the **min / median / max** [[Converted USD Price / USD Price per Unit|USD price per unit]] across that item's **released + approved** [[Quote]]s. That set — Approved quotes whose Country is currently [[Released]] — is the *only* population any dashboard aggregates (#14), always within a single [[Pricing Study]]. Each Quote is one data point: no collapsing by [[Competitor]] or dealer first (a heavily-sourced competitor legitimately weights the spread), and a quote with no per-unit figure is excluded. The **median** is the mean of the middle pair for an even-sized population. An item left with **no usable data points** yields an explicit *no-data* result, not zeros and not a vanished row — it was released, so the client still sees it. Surfaces three ways: the client **range view** (min/median/max per item); the client **by-competitor breakdown**, the same range partitioned by [[Competitor]] within the item (a null brand forms an explicit *unspecified* group); and the internal **benchmark comparison**, the same range set beside the item's [[Client Price]] — *not comparable* when the item has none (mirroring [[Price Flag]]), and **never shown to clients** (ADR-0003). The internal comparison shares the client population (released + approved, never pre-release) — pre-release QC stays per-quote via the [[Price Flag]].
